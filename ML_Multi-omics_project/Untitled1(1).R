...
CNA_sort <- CNA %>% apply(2, abs) %>% apply(2, sum) %>% order(decreasing=TRUE) 
CNA_new  <- CNA[, CNA_sort[0:10]]

SM_sort <- SM %>% apply(2, abs) %>% apply(2, sum) %>% order(decreasing=TRUE) 
SM_new  <- SM[, SM_sort[0:10]]

PE_sort <- PE %>% apply(2, abs) %>% apply(2, sum) %>% order(decreasing=TRUE) 
